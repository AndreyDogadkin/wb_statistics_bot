...
@set_account_router.callback_query(
    StateFilter(AccountsStates.change_account), AccountsCallbackData.filter()
)
async def change_account(
    ...
    is_updated: bool = await database.change_active_account(
        telegram_id=user_id, select_account_id=select_account_id
    )
    user_accounts = await database.get_user_accounts(telegram_id=user_id)
    if is_updated:
        await callback.message.edit_text(
            account_message_templates['main_accounts'],
            reply_markup=MakeMarkup.account_markup(
                user_accounts, gateway=True
            ),
        )
...
@set_account_router.callback_query(
    StateFilter(AccountsStates.change_account), F.data == 'add'
)
async def set_add_account_state(
    ...
    if in_limit:
        for_edit_mess = await callback.message.edit_text(
            account_message_templates['send_account_name'].format(
        ...
        await callback.answer(
            account_message_templates['accounts_limit_alert'], show_alert=True
        )


@set_account_router.message(
    StateFilter(AccountsStates.add_account),
    F.func(lambda x: len(x.text) <= Lengths.MAX_LEN_ACCOUNT_NAME),
)
async def add_new_account(message: types.Message, state: FSMContext):
    ...
    is_created: bool = await database.create_user_account(
        telegram_id=user_id, account_name=account_name
    )
    await message.delete()
    if is_created:
...
@set_account_router.callback_query(
    StateFilter(AccountsStates.change_account, AccountsStates.edit_account),
...
@set_account_router.callback_query(
    StateFilter(AccountsStates.edit_account), AccountsEditCallbackData.filter()
)
async def get_editable_account(
    ...
    for_edit_mess = await callback.message.edit_text(
        account_message_templates['send_new_account_name'].format(
            Lengths.MAX_LEN_ACCOUNT_NAME
        ),
        reply_markup=MakeMarkup.cancel_builder().as_markup(),
    )
    ...
    await state.update_data(
        editable_account_id=editable_account_id, for_edit_mess=for_edit_mess
    )
...
async def get_new_name_for_editable_account(
    ...
    if is_edited:
        await for_edit_mess.edit_text(
            account_message_templates['account_name_edit_done'].format(
                new_account_name
            )
        )
        await state.clear()
...
@set_account_router.callback_query(
    StateFilter(AccountsStates.change_account, AccountsStates.delete_account),
...
async def delete_account(
    ...
    is_deleted = await database.delete_account(
        telegram_id=user_id, account_id=delete_account_id
    )
    if is_deleted:
        await callback.message.edit_text(
