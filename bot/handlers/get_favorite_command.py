...
async def send_favorites_and_update_in_state_data(
    state: FSMContext, callback: types.CallbackQuery | types.Message
):
...
@get_favorite_router.message(
    Command(commands='favorites'), StateFilter(default_state)
)
...
@get_favorite_router.callback_query(
    StateFilter(FavoritesStates.get_favorite, FavoritesStates.delete_favorite),
    ...
    if favorites:
        if callback.data == 'delete':
            ...
            markup = MakeMarkup.favorites_markup(
                favorites=favorites, delete=True
            )
            await callback.message.edit_text(
...
async def delete_favorite(
    ...
    await database.delete_user_favorite(
        telegram_id=user_id, nm_id=nm_id, period=period
    )
...
@get_favorite_router.callback_query(
    StateFilter(FavoritesStates.get_favorite), FavoritesCallbackData.filter()
)
