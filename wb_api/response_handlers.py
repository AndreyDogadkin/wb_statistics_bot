...
class ResponseHandlers:
    ...
    def analytic_detail_period_handler(cls, response):
        ...
        try:
            ...
            statistics.selected_period.begin = cls.__get_datetime_format(statistics.selected_period.begin,
                                                                         'detail_period')
            statistics.selected_period.end = cls.__get_datetime_format(statistics.selected_period.end,
                                                                       'detail_period')
            statistics.previous_period.begin = cls.__get_datetime_format(statistics.previous_period.begin,
                                                                         'detail_period')
            statistics.previous_period.end = cls.__get_datetime_format(statistics.previous_period.end,
                                                                       'detail_period')
            ...
            previous_period = list(statistics.previous_period.__dict__.values())
            previous_period_buyouts: ConversionsStatsPeriod = previous_period.pop()
            return [(name, vendor_code), ('Выбранный период:', *select_period, select_period_buyouts.buyout_percent),
                    ('Предыдущий период:', *previous_period, previous_period_buyouts.buyout_percent)]
        except ValidationError as e:
            logger.critical(f'Ошибка валидации ответа: {e.json()}')
            raise ForUserException(err_mess_templates['response_validation_error'])
